...
class OpenTelemetry(CustomLogger):
    ...
    def _emit_semantic_logs(self, kwargs, response_obj, span: Span):
        ...
        # Resolve SdkLogRecord across OTEL SDK versions.
        #
        # NOTE: In OTEL SDK >= 1.39.0, LogRecord moved to
        # `opentelemetry.sdk._logs._internal`. The shipped type stubs reflect
        # this, so `from opentelemetry.sdk._logs import LogRecord` fails mypy
        # with [attr-defined] even if wrapped in try/except.
        #
        # Use getattr-based lookup to avoid direct attribute import, keeping the
        # lint workflow green while still supporting older OTEL versions.
        import opentelemetry.sdk._logs as otel_sdk_logs

        SdkLogRecord: Any = getattr(otel_sdk_logs, "LogRecord", None)
        if SdkLogRecord is None:
